...
1. **Never custody private keys** — with ONE narrowly-scoped exception. By default all signing happens in external wallets (JoyID, Ledger, MetaMask, WalletConnect) and ChainPay coordinates partial signatures only. **Exception (added 2026-06-28):** a self-custodied BIP39 keystore signer is permitted ONLY on the non-treasury single-sig SMB *send* path. Its seed is encrypted at rest (Argon2id + ChaCha20-Poly1305 in a Rust→WASM vault) and is decrypted, used, and zeroized entirely inside the Electron **main** process — key material never crosses IPC to the renderer, and `signTxInVault` recomputes the sighash digest + verifies the source lock-args match the vault's derived key before signing (never blind-signs). This signer (`kind: "local-keystore"`) is structurally barred from the treasury/multisig path: `SignerKind` excludes it, a `@ts-expect-error` invariant breaks `tsc` if that's ever widened, and `assertNotLocalKeystoreSigner` is the runtime backstop. **The treasury trust root stays multisig/external — Rule #4 is unchanged.**
